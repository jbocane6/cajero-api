...
api = FastAPI()#se crea la API

from fastapi.middleware.cors import CORSMiddleware

origins = [
    "http://localhost.tiangolo.com", "https://localhost.tiangolo.com",
    "http://localhost", "http://localhost:8080", "http://localhost:8081", "http://localhost:8082",
]
...
@api.post("/user/auth/")
async def auth_user(user_in: UserIn):#se implementa auth_user
...
async def get_balance(username: str):#se implementa get_balance
...
async def make_transaction(transaction_in: TransactionIn):# se implementa make_transaction
